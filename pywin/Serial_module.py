...
#        self.About_Button.clicked.connect(self.Goto_GitHub)
    ...
    def HandleData(self, rev_data):
        #pass
        press = int((rev_data[1]&0x3f)<<8) + int(rev_data[2])
        temp = int((rev_data[3]&0x7f)<<8)+ int(rev_data[4])
        return press, temp
    ...
    def Com_Receive_Data(self):
        ...
        try:
            ...
            press, temp = self.HandleData(rxData)

            self.Com_Pressure.setText(str(press)+"Kpa")
